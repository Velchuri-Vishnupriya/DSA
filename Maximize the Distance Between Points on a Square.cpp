...
//optimized Binary search with improved check function
...
bool check(vector<ll>& doubled, int n, int k, int side, int mid){
    ...
    for(int i = 0; i < n; i++){//O(n)
        ...
        for(int j = 2; j <= k; j++){//O(k)
            ...
            auto it = lower_bound(begin(doubled) + idx + 1, begin(doubled) + i + n, target); //O(log n)
        ...
        if(count == k && (doubled[i] + perimeter - lastPos) >= mid)return true;
    }
    return false;
}
int maxDistance(int side, vector<vector<int>>& points, int k) {
...
//double the positions array to handle round wrap
