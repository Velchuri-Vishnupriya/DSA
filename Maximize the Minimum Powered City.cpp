//T.C : O(n* log(Sum))
//S.C : O(n)
int n;
bool check(long long mid, vector<long long>& diff, int r, long long k){//O(n)
    vector<long long> tempDiff = diff;
    
    long long cumSum = 0; //generates the power of a city at any point

    for(int i = 0; i < n; i++){
        cumSum += tempDiff[i];

        if(cumSum < mid){
            long long need = mid - cumSum;
            if(need > k) return false;

            k-=need;

            cumSum += need; //grredily we choose the best possible station to add this need so that we get max benefit

            //i....X....i+2*r

            if(i + 2*r + 1 < n){
                tempDiff[i + 2*r + 1] -= need;
            }
        }
    }
   return true; 
}
class Solution {
public:
    long long maxPower(vector<int>& stations, int r, long long k) {
   n = stations.size();

   vector<long long> diff(n,0);

   for(int i = 0; i < n; i++){
    diff[max(0,i-r)] += stations[i];
    if(i + r + 1 < n){
        diff[i + r + 1] -= stations[i];
    }
   }

   long long left = *min_element(begin(stations), end(stations));
   long long right = accumulate(begin(stations), end(stations), 0LL) + k;

   long long result = 0;

//O(Sum)
   while(left <= right){
    long long mid = left + (right - left)/2;

    if(check(mid, diff, r, k)){
        result = mid;
        left = mid + 1;
    }else{
        right = mid - 1;
    }
   }

   return result;
    }
};
